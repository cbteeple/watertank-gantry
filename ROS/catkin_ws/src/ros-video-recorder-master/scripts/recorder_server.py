...
def cam_do(req):
    ...
    num_videos=1 #ADDED FOR DEBUGGING
...
def camera_server():
	rospy.init_node('video_recorder_'+location, anonymous=True)
	while not rospy.is_shutdown():
		while rospy.get_param('cameras_on')==0:
			if rospy.get_param('cameras_on')==1:
				break
		cam_do('something')
