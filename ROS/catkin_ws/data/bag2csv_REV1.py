...
for bagFile in listOfBagFiles:
			...
			for subtopic, msg, t in bag.read_messages(topicName):	# for each instant in time that has data for topicName
				...
				if firstIteration:	# header
					...
					for pair in instantaneousListOfData:
						if "[" in pair[1]:  #if the data is actually an array
							for idx in range(len(ROSstring2array(pair[1]))):
								headers.append(pair[0]+'_'+str(idx))
						else:
				...
				for pair in instantaneousListOfData:
					if len(pair) !=1:
						if "[" in pair[1]:
							values.extend(ROSstring2array(pair[1]))
						else:		
	...
	bag.close()
print "Woohoo! All " + numberOfFiles + " bag files were converted."
